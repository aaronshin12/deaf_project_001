        ...
        <div className="max-w-app mx-auto">
          ...
          <div className="flex justify-end pt-3">
            <button
              onClick={() => {
                if (navigator.share) {
                  navigator.share({ title: "수어학습 도우미", url: window.location.origin });
                } else {
                  navigator.clipboard.writeText(window.location.origin);
                  alert("링크가 복사되었습니다!");
                }
              }}
              className="w-10 h-10 rounded-full bg-[#2D2B2A] border-2 border-accent flex items-center justify-center hover:bg-[#3D3B3A] transition-colors"
              aria-label="공유하기"
            >
