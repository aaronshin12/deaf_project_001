...
export default function WeekCard({
  ...
  return (
    <Link href={`/week/${id}`}>
      <div className="bg-card border-2 border-card-border rounded-card p-4 hover:border-green transition-colors active:scale-[0.98] transition-transform">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex-shrink-0 w-10 h-10 rounded-full bg-green/10 flex items-center justify-center">
            <span className="text-sm font-bold text-green">{wordCount}</span>
          </div>
        </div>
        ...
        <div className="flex gap-3 mt-3">
          ...
          {noteCount > 0 && (
            <span className="text-xs text-blue flex items-center gap-1">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor">
