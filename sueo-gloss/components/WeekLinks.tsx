  ...
  return (
    <div className="bg-card border-2 border-card-border rounded-card p-4">
      <p className="text-sm font-semibold text-text-main mb-3 flex items-center gap-2">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="#CE82FF">
      ...
      <div className="flex flex-wrap gap-2">
        {otherWeeks.map((week) => (
          <Link
            key={week.id}
            href={`/week/${week.id}`}
            className="px-3 py-1.5 text-sm rounded-full border-2 border-purple/40 text-purple hover:bg-purple/10 transition-colors"
          >
            {week.title}
          </Link>
        ))}
